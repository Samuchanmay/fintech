...
function TarjetaForm({ tarjeta, onSave, onClose }) {
  ...
  function guardar() {
    ...
    onSave({
      ...form,
      limite, saldo, tasaAnual: tasaAnual, diaCorte, diaPago, pagoMinimo,
    });
  }
  ...
  return (
    <Modal title={tarjeta ? "Editar tarjeta" : "Nueva tarjeta"} onClose={onClose}>
      ...
      <div className="form-row">
        <div className="form-group">
          <label>Banco</label>
          <input value={form.banco} onChange={(e) => actualizar("banco", e.target.value)} placeholder="Ej. BBVA" />
        </div>
        <div className="form-group">
          <label>Color</label>
      ...
      <div className="form-row">
        ...
        <div className="form-group">
          ...
          <input type="number" value={form.saldo} onChange={(e) => actualizar("saldo", e.target.value)} placeholder="15000" />
        </div>
      </div>
      <div className="form-row">
        <div className="form-group">
...
function TarjetasModule({ tarjetas, config, onAdd, onUpdate, onDelete }) {
  ...
  function abrirNuevo() { setEditando(null); setMostrarForm(true); }
  function abrirEditar(t) { setEditando(t); setMostrarForm(true); }
  ...
  return (
    <div>
            ...
            return (
              <div className="card" key={t.id}>
                ...
                <div style={{ fontSize: "12px", color: "var(--text-dim)", marginBottom: "10px" }}>
                  de {FinanceUtils.formatoMoneda(t.limite)} ({FinanceUtils.formatoPorcentaje(util)} usado)
                </div>
      ...
      {mostrarForm && (
        <TarjetaForm tarjeta={editando} onSave={guardar} onClose={() => setMostrarForm(false)} />
      )}
    </div>
  );
}
